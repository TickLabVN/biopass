...
bool checkAntispoofByIRCamera(ICameraCaptureSession* ir_camera_session,
  ...
  try {
    ...
    const bool is_diagnostic = ai_enabled && !ir_model_hard_fail;
    const bool ir_model_passed =
        checkAntispoofByIRCrops(ir_faces, ir_as_model, username, debug_enabled, is_diagnostic);
    if (!ir_model_passed && is_diagnostic) {
      spdlog::debug(
          "FaceAuth: IR anti-spoofing model score failed, but IR face presence was "
          "confirmed; treating IR model score as diagnostic because AI liveness is "
          "enabled and ir_model_hard_fail=false");
      return true;
    }
    return ir_model_passed;
...
bool checkAntispoofByIRCrops(const std::vector<ImageRGB>& ir_faces, FaceAntiSpoofing* face_as,
                             const std::string& username, bool debug, bool is_diagnostic) {
  if (!face_as) {
  ...
  try {
      ...
      if (!is_face_real) {
        if (is_diagnostic) {
          spdlog::debug(
              ...
              "spoof={:.4f}, threshold={:.3f}) (diagnostic)",
    ...
    if (passed) {
      ...
      if (is_diagnostic) {
        spdlog::debug(
            "FaceAuth: IR liveness check AGGREGATE FAILED (diagnostic) — {}/{} strict face(s) "
            "passed, "
            "avg_real={:.4f}, avg_spoof={:.4f}, threshold={:.3f}",
            passed_faces, valid_faces_processed, avg_real_score, avg_spoof_score,
            antispoof_threshold);
