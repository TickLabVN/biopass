...
If the IR pipeline is enabled alongside the RGB AI model, IR face presence is a hard requirement, while the IR model score is diagnostic by default because the bundled anti-spoofing model is RGB-trained rather than IR-domain tuned. Set `anti_spoofing.ir_model_hard_fail: true` to require the IR model score to pass as a hard gate. If the RGB AI model is disabled, the IR model score remains a hard gate. The RGB AI anti-spoofing task and IR liveness task run in parallel.
