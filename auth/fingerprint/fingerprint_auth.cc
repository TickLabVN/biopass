...
void on_verify_status(GDBusConnection *connection, const gchar *sender_name,
  ...
  std::cout << "Fingerprint status: " << res_str << ", done: " << done
            << std::endl;
  ...
  } else if (res_str == "verify-unknown-error" ||
             res_str == "verify-disconnected") {
...
bool FingerprintAuth::is_available() const {
  ...
  GDBusProxy *manager = g_dbus_proxy_new_for_bus_sync(
      G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, nullptr, FPRINT_SERVICE,
      FPRINT_MANAGER_PATH, FPRINT_MANAGER_INTERFACE, nullptr, &error);

  if (error) {
    std::cerr << "Failed to connect to fprintd manager: " << error->message
              << std::endl;
    g_error_free(error);
    return false;
  }
  ...
  GVariant *ret =
      g_dbus_proxy_call_sync(manager, "GetDefaultDevice", nullptr,
...
AuthResult FingerprintAuth::authenticate(const std::string &username,
  ...
  GDBusProxy *manager = g_dbus_proxy_new_sync(
      connection, G_DBUS_PROXY_FLAGS_NONE, nullptr, FPRINT_SERVICE,
      FPRINT_MANAGER_PATH, FPRINT_MANAGER_INTERFACE, nullptr, &error);
  ...
  GVariant *dev_ret =
      g_dbus_proxy_call_sync(manager, "GetDefaultDevice", nullptr,
                             G_DBUS_CALL_FLAGS_NONE, -1, nullptr, &error);
  if (!dev_ret) {
    std::cerr << "No fingerprint device found: " << (error ? error->message : "Unknown") << std::endl;
  ...
  // 3. Get Device Proxy
  GDBusProxy *device = g_dbus_proxy_new_sync(
      connection, G_DBUS_PROXY_FLAGS_NONE, nullptr, FPRINT_SERVICE,
  ...
  GVariant *enrolled_ret = g_dbus_proxy_call_sync(
      device, "ListEnrolledFingers", g_variant_new("(s)", username.c_str()),
      G_DBUS_CALL_FLAGS_NONE, -1, nullptr, &error);
  
  if (!enrolled_ret) {
      std::cerr << "Failed to list enrolled fingers (user might not exist or permission denied): " << (error ? error->message : "") << std::endl;
      if (error) g_error_free(error);
  ...
  if (!has_fingers) {
      ...
      g_object_unref(manager);
       g_object_unref(connection);
      return AuthResult::Unavailable; // Should we return Failure? Unavailable seems appropriate if not set up.
  }

  // 5. Claim Device
  GVariant *claim_ret = g_dbus_proxy_call_sync(
      device, "Claim", g_variant_new("(s)", username.c_str()),
      G_DBUS_CALL_FLAGS_NONE, -1, nullptr, &error);
  ...
  GVariant *verify_ret = g_dbus_proxy_call_sync(
      device, "VerifyStart", g_variant_new("(s)", "any"),
      G_DBUS_CALL_FLAGS_NONE, -1, nullptr, &error);
  ...
  if (!verify_ret) {
      ...
      if (error) g_error_free(error);
      // Release
      GVariant *rel_ret = g_dbus_proxy_call_sync(device, "Release", nullptr, G_DBUS_CALL_FLAGS_NONE, -1, nullptr, nullptr);
      if (rel_ret)
  ...
  // Stop Verification
  GVariant *stop_ret = g_dbus_proxy_call_sync(device, "VerifyStop", nullptr, G_DBUS_CALL_FLAGS_NONE, -1, nullptr, nullptr);
  if (stop_ret)
    g_variant_unref(stop_ret);

  // Release
  GVariant *release_ret = g_dbus_proxy_call_sync(device, "Release", nullptr, G_DBUS_CALL_FLAGS_NONE, -1, nullptr, nullptr);
  if (release_ret)
